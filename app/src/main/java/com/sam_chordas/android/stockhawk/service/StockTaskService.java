...
public class StockTaskService extends GcmTaskService {
    ...
    public int onRunTask(TaskParams params) {
        ...
        //Overtime
        else if (params.getTag().equals("overtime")) {
            isUpdate = false;
            // get symbol from params.getExtra and build query
            ...
            try {
                ...
                urlStringBuilder.append(URLEncoder.encode("select * from yahoo.finance.historicaldata where symbol = \""
                                + stockInput + "\" and startDate = \"" + startDate + "\" and endDate = \"" + endDate + "\""
                        , "UTF-8"));

            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
        } else if (params.getTag().equals("refresh")) {
            try {
                // Base URL for the Yahoo query
                urlStringBuilder.append(URLEncoder.encode("select * from yahoo.finance.quotes where symbol "
                        + "in (", "UTF-8"));
        ...
        try {
            ...
            if (!params.getTag().equals("overtime")) {
                try {
                    ...
                    ArrayList data = Utils.quoteJsonToContentVals(mContext, getResponse);
                    if (data != null) {
                        mContext.getContentResolver().applyBatch(QuoteProvider.AUTHORITY, data);
                    }
                ...
                ArrayList<DetailStockPojo> detailStockVals = Utils.quoteJsonToDetailStockVals(getResponse);

                Intent intent = new Intent("overtime_data");
                intent.putParcelableArrayListExtra("data", detailStockVals);
